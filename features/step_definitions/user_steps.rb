...
When /^I set daily digest preference to "(.*?)"$/ do |receives_digest|
  user = Mentions[User]
  visit "/users/#{user.id}/edit"
  receives_digest == 'true' ? check('Daily digest') : uncheck('Daily digest')
  click_on 'Update User'
end
...
Then /^daily digest preference should be "(.*?)"$/ do |receives_digest|
  user = Mentions[User]
  visit "/users/#{user.id}"
  page.should have_content(receives_digest)
end
